  ...
  if (!loadPromise) {
    loadPromise = new Promise((accept, reject) => {
      ...
      redisClient.auth(redisKey, err => {
        if (!err) {
          const args = `${nftIndexName} PREFIX ${redisPrefixes.mainnetsidechainNft}: SCHEMA id NUMERIC SORTABLE currentOwnerAddress TEXT currentLocation TEXT description TEXT minterAddress TEXT ownerAddress TEXT properties TEXT ext TEXT hash TEXT name TEXT unlockable TEXT`.split(' ');
