...
const _connectRedis = (port, host) =>
    new Promise((accept, reject) => {
        ...
        redisClient.auth(redisKey, (err) => {
            if (!err) {
                const args =
                    `${nftIndexName} PREFIX 1 ${redisPrefixes.mainnetsidechainNft}: SCHEMA id NUMERIC SORTABLE currentOwnerAddress TEXT currentLocation TEXT description TEXT minterAddress TEXT ownerAddress TEXT properties TEXT ext TEXT hash TEXT name TEXT unlockable TEXT`.split(
                        " "
                    );
                console.log("create index 1", args);
                redisClient.ft_create.apply(
                    redisClient,
                    args.concat([
                        (err) => {
