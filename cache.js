...
async function initNftCache({ chainName }) {
    ...
    let o = await getRedisItem(ids.lastCachedBlockNft, redisPrefixes[chainName + "Nft"]);
...
async function initAccountCache({ chainName }) {
    ...
    const o = await getRedisItem(ids.lastCachedBlockAccount, redisPrefixes[chainName + "Account"]);
    const lastBlockNumber = o?.Item?.number || 0;
