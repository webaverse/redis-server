...
const rawSupportedTypes = ["png", "jpg", "gif", "mp3", "mp4", "glb"];
...
const _cancelEntry = (
    ...
    if (candidateWithdrawIndex !== -1 && candidateDepositIndex !== -1) {
        ...
        return [deposits, withdraws, currentLocation, currentAddress, transactionHash];
...
const formatToken =
    ...
    async (
        ...
        let [minter, owner, description, sidechainMinterAddress] = await Promise.all([
            _log(
...
const formatLand = (contractName) => (chainName) => async (token, storeEntries) => {
    ...
    const [description, rarity, extents, sidechainMinterAddress] = await Promise.all([
...
const getChainNft =
    ...
    polygonToken, */
            ...
            (async () => {
                ...
                const { hash } = token;
                let [unlockable, encrypted] = await Promise.all([
